...
import { UserCreateInput, SigninInput, VerifyAccountInput, ResendAccountInput, UserTokenData } from '../../utils/types';
import { UserTokenResponse } from './interface';
import { generateRandomCode, generateToken } from '../../utils/helper';

export const signUp = async (data: UserCreateInput): Promise<string> => {
...
export const signIn = async (data: SigninInput): Promise<UserTokenResponse | null> => {
  const userRepository = getRepository(User);
...
export const verifyAccount = async (input: VerifyAccountInput): Promise<string | undefined> => {
  const userRepository = getRepository(User);
  const otpRepository = getRepository(Otp);
...
export const resendVerification = async (input: ResendAccountInput): Promise<string | undefined> => {
  const userRepository = getRepository(User);
  const otpRepository = getRepository(Otp);
